package vn.edu.ptit.duongvct.discord_bot_test1.common.resource;

public final class CreateResourceCommandCommon {
    public static final String CREATE_RESOURCE_COMMAND_DESCRIPTION = "Create a learning resource";
    public static final String NAME_PARAMETER = "name";
    public static final String DESCRIPTION_PARAMETER = "description";
    public static final String LINK_PARAMETER = "link";
    public static final String COURSE_ID_PARAMETER = "course-id";
    public static final String TOPIC_ID_PARAMETER = "topic-id";
    public static final String TYPE_PARAMETER = "type";

    public static class CreateResourceParameterCommon {
        public static final String NAME_PARAMETER_DESCRIPTION = "Resource name";
        public static final String DESCRIPTION_PARAMETER_DESCRIPTION = "Resource description";
        public static final String LINK_PARAMETER_DESCRIPTION = "Resource link";
        public static final String COURSE_ID_PARAMETER_DESCRIPTION = "Course of the resource";
        public static final String TOPIC_ID_PARAMETER_DESCRIPTION = "Topic of the resource";
        public static final String TYPE_PARAMETER_DESCRIPTION = "Resource type";
    }
}